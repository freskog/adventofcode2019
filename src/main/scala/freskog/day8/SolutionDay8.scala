...
object SolutionDay8 extends App {
  ...
  val layers: ZIO[Console, Nothing, List[Layer]] =
    inputAsString.flatMap(
      parse(_, allLayers(25, 6)(_)).fold(
  ...
  val partOne: ZIO[Console, Nothing, Unit] =
    layers.map(_.minBy(_.count(Pixel(0)))).map(m => m.count(Pixel(1)) * m.count(Pixel(2))).flatMap { answer =>
      console.putStrLn(s"partOne: count of 1s * 2s in 0-est layer is $answer")
    }
